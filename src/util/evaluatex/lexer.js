...
export default function lexer (equation, constants = {}, opts = DEFAULT_OPTS) {
  const l = new Lexer(equation, constants, opts)
  l.lex()
...
class Lexer {
  ...
  lexExpression (charMode = false) {
    while (this.hasNext()) {
      const token = charMode ? this.nextCharToken() : this.next()
  ...
  next (len = undefined) {
    ...
    if (!this.hasNext()) {
      throw Error('Lexer error: reached end of stream')
    }
    ...
    for (const [type, regex] of Token.patterns) {
      ...
      const match = regexFromStart.exec(this.buffer.substr(0, len))
      if (match) {
    ...
    throw Error('Lexer error: can\'t match any token')
  }
