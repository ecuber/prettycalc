...
class Parser {
  ...
  parse () {
    ...
    if (this.currentToken !== undefined) {
      ...
      throw Error('Parsing error: Expected end of input, but got ' + this.currentToken.type +
  ...
  expect (type) {
    if (!this.accept(type)) {
      throw Error('Expected ' + type + ' but got ' +
  ...
  /**
     ...
     */
  sum () {
    const node = new Node(Node.TYPE_SUM)
  ...
  product () {
    const node = new Node(Node.TYPE_PRODUCT)
  ...
  power () {
    const node = new Node(Node.TYPE_POWER)
  ...
  val () {
    ...
    if (this.accept(Token.TYPE_SYMBOL)) {
      ...
      throw Error('Unexpected ' + this.currentToken.toString() + ' at token ' + this.cursor)
    }

    if (this.accept(Token.TYPE_BANG)) {
      const factNode = new Node(Node.TYPE_FUNCTION, fact)
